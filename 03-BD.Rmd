---
title: "DADA2 tutorial - Big Data (SE) reads"
output:
...
```{r, eval=params$process}
set.seed(100)
err <- learnErrors(fastqs_filt, nbases = 1e8, multithread = n_cores, verbose = TRUE, randomize=TRUE)
```

Run sample inference.

```{r, eval=params$process}
dds <- vector("list", length(sampleIDs))
names(dds) <- sampleIDs

for(sam in sampleIDs) {
  cat("Processing of file", sam, "begins on:", format(Sys.time(), "%c"), "\n")
  derep <- derepFastq(fastqs_filt[[sam]])
...
track <- cbind(filter_results, sapply(dds, getN), rowSums(seqtab))
...
n <- 1000 #number of ASVs for each block
nr <- nrow(df)
